...
insults = [
    ...
    'has got problems. Many respected people say so.',
    ...
    'is killing us! It\'s so terrible.',
    'must be stopped, but weak Washington won\'t act.',
    ...
    'is very soft! Has no strength at all.',
    'has a major inferiority complex.',
    'is such a dopey clown.',
]
endings = [
    'SAD',
    'WOW',
...
def check_size(image):
    if int(image['data-file-width']) >= 350:
        return True
...
def write_tweet(title):
    if any(char.isdigit() for char in title):
        text = "Do people really think %s was any good? %s!" % (title, random.choice(endings))
    else:
